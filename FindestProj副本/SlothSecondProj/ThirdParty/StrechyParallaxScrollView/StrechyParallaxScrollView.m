...
{
    if ([self.delegate respondsToSelector:@selector(strechyParallaxDidScroll:)])
    {
        [self.delegate strechyParallaxDidScroll:scrollView];
    }
    ...
    if (scrollView.contentOffset.y < 0)
    {
        ...
        [self.topView setFrame:CGRectMake(0, 0, self.topView.frame.size.width, newH)];
...
{
    if ([self.delegate respondsToSelector:@selector(strechyParallaxWillBeginDraggin:)])
    {
        [self.delegate strechyParallaxWillBeginDraggin:scrollView];
    }
}
