...
//#import "PublishShareOldViewController.h"
#import "Utils.h"
...
- (void)loadView
{
    ...
    UIColor *backgroundColor = [dataSource_ respondsToSelector:@selector(imageBackgroundColor)] ? [dataSource_ imageBackgroundColor] : [UIColor blackColor];
...
- (void)loadPhoto:(NSInteger)index
{
    ...
    if (NO == [currentPhotoView isKindOfClass:[KTPhotoView class]])
    {
        ...
        //弹出操作表
        UILongPressGestureRecognizer *longPressGesture = [[[UILongPressGestureRecognizer alloc]init]autorelease];
        [longPressGesture addTarget:self action:@selector(popupActionSheet:)];
...
//        UIImage *imgOrign = photoView.imageView.image;
//        UIImage *imgThumb = imgOrign;
//        
//        //save photo
//        NSData *imageData = UIImageJPEGRepresentation(imgOrign, 1.0f);
...
//        {
//            //分享
//            PublishShareOldViewController *publishShareViewController = [[PublishShareOldViewController alloc]init];
//            
...
//保存到系统相册
-(void)popupActionSheet:(UILongPressGestureRecognizer*)gestureRecognizer
{
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if (error == nil)
    {
        [Common bubbleTip:@"已保存到系统相册" andView:self.view];
    }
    else
    {
        
        [Common bubbleTip:[NSString stringWithFormat:@"保存失败，%@",[error localizedDescription]] andView:self.view];
    }
}

-(void)updatePraiseAndCommentNum:(NSInteger)nIndex
{
    //    if (nIndex >= 0)
    //    {
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex 
{
    ...
    else
    {
        ...
        if (buttonIndex == 0)
        {
            KTPhotoView *photoView = (KTPhotoView *)[scrollView_ viewWithTag:actionSheet.tag];
            UIImageWriteToSavedPhotosAlbum(photoView.imageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        }
    }
}
