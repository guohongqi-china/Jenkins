...
static NSString *g_updateVersionURL;                     //跟新地址
...
__strong static SuggestionBaseVo *g_suggestionBaseVo = nil;         //保存合理化建议基础数据信息
...
//保存合理化建议基础数据信息
+ (void)setSuggestionBaseVo:(SuggestionBaseVo*)suggestionBaseVo
{
    if (g_suggestionBaseVo != suggestionBaseVo)
    {
        g_suggestionBaseVo = suggestionBaseVo;
    }
}
+ (SuggestionBaseVo*)getSuggestionBaseVo
{
    return g_suggestionBaseVo;
}
+ (void)setUpdateVersionURL:(NSString *)strURL{
    if (g_updateVersionURL != strURL) {
        g_updateVersionURL = [strURL copy];
    }
    
}
+ (NSString *)getUpdateVersionURL{
    return g_updateVersionURL;
}
//获取登录用户的密码
...
+ (NSString *)getDateWeekStrFromDate:(NSDate *)date
{
    ...
    switch (compts.weekday) {
        ...
        case 7:
            ...
            break;
        default:
            strWeek = [NSString stringWithFormat:@"%li",(long)compts.weekday];
            break;
    }
...
+(void)initGlobalValue
{
    ...
    else if([[[UIDevice currentDevice] systemVersion] compare:@"10.0"] != NSOrderedAscending)
    {
        //iOS7
        iOSPlatform	= 10;
        kScreenWidth	= [[UIScreen mainScreen] bounds].size.width;
...
{
    [Common showProgressView:strText view:view offset:-30 modal:bModal];
}

+(void)showProgressView:(NSString*)strText view:(UIView*)view offset:(NSInteger)nOffset modal:(BOOL)bModal
{
