...
//获取小视频路径:Documents/Sloth2Document/SmallVideo
+ (NSString *)getAppSmallVideoPath
{
    NSString *strTempPathDir = [[Utils documentsDirectory] stringByAppendingPathComponent:SLOTH2_SMALL_VIDEO_PATH];
...
{
    NSNumber *numFileSize = nil;
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSDictionary *fileAttributes = [fileManager attributesOfItemAtPath:filePath error:nil];
    if(fileAttributes != nil)
    {
        numFileSize = [fileAttributes objectForKey:NSFileSize];
    }
...
//创建UIBarButtonItem按钮
+ (UIButton *)buttonWithImage:(UIImage *)imgBtn frame:(CGRect)rect target:(id)target action:(SEL)action
{
...
+ (UIButton *)buttonWithTitle:(NSString*)strTitle frame:(CGRect)rect target:(id)target action:(SEL)action
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = rect;
    [button setTitleColor:[SkinManage colorNamed:@"Nav_Btn_Color"] forState:UIControlStateNormal];
...
+ (UIButton *)leftButtonWithTitle:(NSString*)strTitle frame:(CGRect)rect target:(id)target action:(SEL)action
{
    ...
    [button setBackgroundImage:[[SkinManage imageNamed:@"nav_left_btn"] stretchableImageWithLeftCapWidth:45 topCapHeight:0] forState:UIControlStateNormal];
    [button setTitle:strTitle forState:UIControlStateNormal];
    [button addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    return button;
}
