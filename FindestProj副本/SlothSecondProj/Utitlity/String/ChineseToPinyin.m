...
+ (NSArray *)getQPAndJPFromChiniseString:(NSString *)string
{
    ...
    for (int i=0; i< iLen; i++)
    {
        if ( (unsigned char)gb2312_string[i] < 0x80 )
		{
            //非汉字
			strValue = [strValue stringByAppendingFormat:@"%c", gb2312_string[i] > 95 ? gb2312_string[i]-32 : gb2312_string[i] ];
			strSimpleValue = [strSimpleValue stringByAppendingFormat:@"%c", gb2312_string[i] > 95 ? gb2312_string[i]-32 : gb2312_string[i] ];
            continue;
		}
        ...
        {
            strSimpleValue = [strSimpleValue stringByAppendingString:[strRes substringToIndex:1]];
        }
        i++;
    }
	return [NSArray arrayWithObjects:strValue,strSimpleValue,nil];	
}
