...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) 
    {
        ...
        self.imgViewHead.layer.cornerRadius = 5;
        self.imgViewHead.layer.masksToBounds = YES;
        ...
        self.lblName.textColor = [SkinManage colorNamed:@"Chat_Name_Label"];
        ...
        //4.1 chat content
        self.txtViewContent = [[FaceLabel alloc] initWithFrame:CGRectZero andFont:[UIFont systemFontOfSize:16]];
        ...
        self.lblDateTime.textColor = [SkinManage colorNamed:@"Notify_Date_Color"];// COLOR(191, 180, 177, 1.0);
        ...
        self.lblAttachmentName.font = [UIFont systemFontOfSize:16];
        ...
        self.lblAudioDuration.textColor = [SkinManage colorNamed:@"myjob_Button_title_color"];
        ...
        [self.lblChatNotification.layer setBorderWidth:0.5];
        [self.lblChatNotification.layer setCornerRadius:5];
        self.lblChatNotification.layer.borderColor = COLOR(207, 207, 207, 1.0).CGColor;
        ...
        self.lblChatNotification.lineBreakMode = NSLineBreakByWordWrapping;
...
- (void)initWithChatContent:(ChatContentVo *)chatContentVo
{
    ...
    CGSize labelSize = [Common getStringSize:strChatDateTime font:lblDateTime.font bound:CGSizeMake(MAXFLOAT, 20) lineBreakMode:self.lblDateTime.lineBreakMode];
    ...
    NSDate *dateLastChat = [Common getDateFromString:self.strLastChatTime format:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *dateNowChat = [Common getDateFromString:chatContentVo.strChatTime format:@"yyyy-MM-dd HH:mm:ss"];
    ...
    if (chatContentVo.nChatType == 1) 
    {
        ...
        if([chatContentVo.strVestId isEqualToString:[Common getCurrentUserVo].strUserID])
        {
            ...
            self.txtViewContent.textColor = [SkinManage colorNamed:@"Function_BK_Color"];
            //[UIColor whiteColor];
        }
        else
        {
            ...
            self.txtViewContent.textColor = [SkinManage colorNamed:@"Menu_Title_Color"];
        }
    }
    else
    {
        ...
        {
            //other
            [self.imgViewHead sd_setImageWithURL:[NSURL URLWithString:chatContentVo.strHeadImg] placeholderImage:phImageName];
            self.txtViewContent.textColor = [SkinManage colorNamed:@"Menu_Title_Color"];
        }
    }
    ...
    //4.聊天文本内容
    if(chatContentVo.nContentType == 0)
    {
        ...
        if ([chatContentVo.strVestId isEqualToString:[Common getCurrentUserVo].strUserID])
        {
            ...
            self.txtViewContent.frame = CGRectMake(self.imgViewChatBK.frame.origin.x+15, fHeight+14,fContentWidth,fContentHeight);
        }
        else
        {
            //others(left side)
            ...
            self.txtViewContent.frame = CGRectMake(self.imgViewChatBK.frame.origin.x+20, fHeight+14,fContentWidth,fContentHeight);;
        }
        //设置内容是为了赋值
...
+ (CGFloat)calculateCellHeight:(ChatContentVo *)chatContentVo andLastChatTime:(NSString*)strLastTime
{
    ...
    fHeight += 11;
    //比较与上次的聊天时间间隔是否在一分钟内，若是则不显示时间
    NSDate *dateLastChat = [Common getDateFromString:strLastTime format:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *dateNowChat = [Common getDateFromString:chatContentVo.strChatTime format:@"yyyy-MM-dd HH:mm:ss"];
    if (dateLastChat != nil && dateNowChat != nil)
    {
        NSTimeInterval timeInterval = [dateNowChat timeIntervalSinceDate:dateLastChat];
    ...
    {
        //纯文本
        FaceLabel *lblText = [FaceLabel getFaceLabelInstanceWithFont:[UIFont systemFontOfSize:16]];
    ...
    {
        CGSize sizeFileName = [Common getStringSize:chatContentVo.strFileName font:[UIFont systemFontOfSize:16] bound:CGSizeMake(CHAT_CONTENT_WIDTH-30, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
        if(sizeFileName.height <30)
        {
            fHeight += 15+20 + 30;
        }
    ...
    else if (chatContentVo.nContentType == 4)
    {
        //通知（有人加入和退出群聊）
        fHeight += 2;
        CGSize size = [Common getStringSize:chatContentVo.strContent font:[UIFont systemFontOfSize:14] bound:CGSizeMake(kScreenWidth-48-20, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
...
{
    //附件没有复制功能
    if (self.m_chatContentVo.nContentType != 2 && self.m_chatContentVo.nContentType != 3)
    {
...
-(void)setImageContent:(int)nDirection
{
    ...
    else
    {
        ...
        if(m_chatContentVo.imgChatTemp == nil)
        {
            ...
            [self.imgViewChat sd_setImageWithURL:[NSURL URLWithString:strImgPath] placeholderImage:imgTemp options:0 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (image)
                {
...
-(CGFloat)getAudioBKLength:(NSString*)strLength
{
    ...
    //定义时间范围在1~60秒乘以10（10~600）(采用对数方案)log600 = 2.778;
    //标准值,66是最小宽度
