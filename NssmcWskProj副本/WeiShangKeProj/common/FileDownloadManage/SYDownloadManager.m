...
- (void)downloadFile:(NSString*)strURL delegate:(id<DownloadManagerDelegate>)delegate
{
    ...
    [operation downloadFileWithProgressBlock:^(SYHTTPRequestOperation *request, unsigned long long totalBytesRead, unsigned long long totalBytesExpectedToRead){
        //不能使用operation.delegate,会造成循环引用
        [request.delegate updateDownloadProgress:request bytesOfRead:totalBytesRead bytesOfExpected:totalBytesExpectedToRead];
...
{
    [operation resumeDownloadOperation];
}

//暂停下载文件
- (void)pauseDownload:(SYHTTPRequestOperation*)operation
{
    //将暂停下载定位停止下载，AFNetworking暂停功能失效
    //[operation pauseDownloadOperation];
