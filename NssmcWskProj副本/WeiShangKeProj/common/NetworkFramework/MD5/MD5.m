...
MD5 MD5WithBytes(const void* bytes, NSUInteger length)
{
	...
	if(length > 16) {
		CC_MD5(bytes, (unsigned int)length, md5.bytes);
		return md5;
	}
...
NSString * StringToMD5StringWithBase64Encoding(NSString *str)
{
	const char *cStr = [str UTF8String];
    unsigned char result[16];
	for( int i =12;i<16;i++)
	...
	//Base64 Encoding
    char base64Result[24];
    size_t theResultLength = 24;
    Base64EncodeData(result, 16, base64Result, &theResultLength);
    NSData *theData = [NSData dataWithBytes:base64Result length:theResultLength];
   
    NSString *base64EncodedResult = [[NSString alloc] initWithData:theData encoding:NSUTF8StringEncoding];
	
    //modify by sunyan 2012-11-27
	//NSLog(@"base64 = %@",base64EncodedResult);
