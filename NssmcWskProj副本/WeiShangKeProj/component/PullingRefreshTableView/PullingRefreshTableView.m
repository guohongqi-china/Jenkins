...
//        self.backgroundColor = [UIColor clearColor];
        UIFont *ft = [UIFont systemFontOfSize:13.f];
        ...
        _stateLabel.text = [Common localStr:@"RefreshTableView_Pull" value:@"下拉可以刷新..."];
...
//    x = 0;
    ...
    if (self.isAtTop) {
        ...
        _arrow.contents = (id)arrow.CGImage;
        _stateLabel.text = [Common localStr:@"RefreshTableView_PullUp" value:@"上拉可以加载更多..."];
    ...
    if (!self.isAtTop) {
        _activityView.center = CGPointMake(kPROffsetY, kPROffsetYFoot/2);
    }
...
- (void)setState:(PRState)state animated:(BOOL)animated{
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            _loading = YES;
            if (self.isAtTop) {
                _stateLabel.text = [Common localStr:@"RefreshTableView_Loading" value:@"载入中..."];
            } else {
                _stateLabel.text = [Common localStr:@"RefreshTableView_Loading" value:@"载入中..."];
            }
            ...
            if (self.isAtTop) {
                _stateLabel.text = [Common localStr:@"RefreshTableView_Release" value:@"松开即可刷新..."];
            } else {
                _stateLabel.text = [Common localStr:@"RefreshTableView_ReleaseLoading" value:@"松开加载更多..."];
            }
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = [Common localStr:@"RefreshTableView_Loaded" value:@"已全部加载完毕"];
            }
        }
    }
}
...
- (void)updateRefreshDate :(NSDate *)date{
    ...
    NSString *title = [Common localStr:@"RefreshTableView_Today" value:@"今天"];
    ...
    if (year == 0 && month == 0 && day < 3) {
        if (day == 0) {
            title = [Common localStr:@"RefreshTableView_Today" value:@"今天"];
        } else if (day == 1) {
            title = [Common localStr:@"RefreshTableView_Yesterday" value:@"昨天"];
        } else if (day == 2) {
            title = [Common localStr:@"RefreshTableView_BYesterday" value:@"前天"];
        }
        df.dateFormat = [NSString stringWithFormat:@"%@ HH:mm",title];
        dateString = [df stringFromDate:date];
        
    }
    _dateLabel.text = [NSString stringWithFormat:@"%@: %@",[Common localStr:@"RefreshTableView_Last" value:@"最后更新"],dateString];
    [df release];
}
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        rect = CGRectMake(0, frame.size.height, frame.size.width, kPROffsetYFoot);
...
- (void)setHeaderHidden:(BOOL)bHidden
{
    _headerView.hidden = bHidden;
}

- (void)setFooterHidden:(BOOL)bHidden
{
    _footerView.hidden = bHidden;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    } else if ( yMargin > kPROffsetYFoot){  //footer totally appeared
        if (_footerView.state != kPRStateHitTheEnd) {
            _footerView.state = kPRStatePulling;
        }
    } else if ( yMargin < kPROffsetYFoot && yMargin > 0) {//footer part appeared
        if (_footerView.state != kPRStateHitTheEnd) {
            _footerView.state = kPRStateNormal;
        }
    }
...
//    } else  if (offset.y + size.height - contentSize.height > kPROffsetY){
        ...
        [UIView animateWithDuration:kPRAnimationDuration animations:^{
            self.contentInset = UIEdgeInsetsMake(0, 0, kPROffsetYFoot, 0);
        }];
