 ...
 withConnectionTimeout:(NSTimeInterval)connectionTimeout
{
    if (!_isConnected && !_isConnecting) {
        ...
        //增加nsid (add by sunyan 2015-02-06)
        //handshakeUrl = [NSString stringWithFormat:@"%@%@",handshakeUrl,[ServerURL getFormatChatSessionID]];
...
- (void) disconnectForced
{
    ...
    //增加nsid (add by sunyan 2015-02-06)
    urlString = [NSString stringWithFormat:@"%@%@",urlString,[ServerURL getFormatChatSessionID]];
            ...
            case 6: {
                ...
                if ([pieces count] > 0) {
                    ...
                    if (argsStr && ![argsStr isEqualToString:@""]) {
                        ...
                        if (!_returnAllDataFromAck && [(NSArray*)argsData count] > 0) {
