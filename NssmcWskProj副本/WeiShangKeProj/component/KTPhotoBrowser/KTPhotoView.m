...
//    YLGIFImage *ylGIFImage = [[YLGIFImage alloc] initWithData:data];
//    self.imgViewGIF = [[[YLImageView alloc] initWithImage:ylGIFImage]autorelease];
//    [self.imgViewGIF setContentMode:UIViewContentModeScaleAspectFit];
//    self.imgThumb = [ylGIFImage getFrameWithIndex:0];//获取第一帧图片，用于分享和转发图片
//    self.dataGIF = data;
//    self.imgViewGIF.frame = self.bounds;
//    [self addSubview:self.imgViewGIF];
//
//    [ylGIFImage release];
}
...
//    if ([self isZoomed] == NO && CGRectEqualToRect([self bounds], [self.imgViewGIF frame]) == NO)
//    {
//        [self.imgViewGIF setFrame:[self bounds]];
//    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
   ...
   if ([touch view] == self)
   {
      if ([touch tapCount] == 1)
      {
         [self performSelector:@selector(toggleChromeDisplay) withObject:nil afterDelay:0.3];
      }
   }
}
...
{
   UIView *viewToZoom = nil;
    if (self.bIsGIF)
    {
        //viewToZoom = self.imgViewGIF;
    }
    else
    {
        viewToZoom = imageView_;
    }
